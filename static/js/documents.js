document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById("queryButton").addEventListener("click", function(event) {
        ...
        tableRows.forEach(function(row) {
            var documentDate = row.querySelector('td:nth-child(3)').innerHTML;
            var documentTitle = row.querySelector('td:nth-child(1)').innerHTML;
            if (
                (queryDate === "" || documentDate === queryDate) &&
                (queryTitle === "" || documentTitle === queryTitle)
            ) {
                row.style.display = '';
            } else {
                row.style.display = 'none';
